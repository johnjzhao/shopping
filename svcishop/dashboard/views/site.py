...
from svcishop.account.utils import admin_required
from svcishop.checkout.models import ShippingMethod
from svcishop.dashboard.forms import (
    DashboardMenuForm,
    ShippingMethodForm,
    SiteMenuForm,
    SitePageForm,
...
from svcishop.dashboard.utils import wrap_partial, item_del
from svcishop.dashboard.models import DashboardMenu, Setting
from svcishop.plugin.models import PluginRegistry
from svcishop.product.models import Category, Collection
from svcishop.public.models import MenuItem, Page
...
def plugin_enable(id):
    ...
    plugin.save()
    flash(
        lazy_gettext("The plugin is enabled, Please restart svci-shop now!"), "success"
    )
...
def plugin_disable(id):
    ...
    plugin.save()
    flash(
        lazy_gettext("The plugin is disabled, Please restart svci-shop now!"), "info"
    )
    return redirect(url_for("dashboard.plugin_list"))
